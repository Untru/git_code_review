...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.КоммитыВерсия.Видимость = Объект.ОпределятьВерсиюХранилища;
...
&НаКлиенте
Процедура ОпределятьВерсиюХранилищаПриИзменении(Элемент)
...
Процедура ConsoleOnClick(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если НЕ ТолькоПросмотр Тогда
		СтандартнаяОбработка = Ложь;
		Если ДанныеСобытия.Свойство("href") И ЗначениеЗаполнено(ДанныеСобытия.href) И СтрНайти(ДанныеСобытия.href, "null") = 0  Тогда
...
Процедура ПослеУдаленияВременныхФайлов(ДопПараметры) Экспорт
	...
	ДанныеМакета = ПолучитьМакет(?(ЭтоLinux, "src_linux", "src_win"));
...
Функция ЗапуститьКомандуGit(Команда)
	...
	КаталогКоманды = БазовыйКаталогРепозитория;
	
	Если Не ЗначениеЗаполнено(КаталогКоманды) Тогда
		КаталогКоманды = Объект.КаталогРепозитория;
	КонецЕсли;
		
	Возврат ЗапуститьКоманду("git " + Команда, КаталогКоманды);
...
&НаКлиенте
Процедура ЗаполнитьБазовыйКаталогРепозитория()
	
	Результат = ЗапуститьКомандуGit("rev-parse --show-toplevel");
	
	Если Результат.КодВозврата = 0 Тогда
		БазовыйКаталогРепозитория = СтрЗаменить(СокрП(Результат.ТекстВывода), "/", РазделительПути);
	Иначе
		БазовыйКаталогРепозитория = Объект.КаталогРепозитория;
		ТекстОшибки = НСтр("ru = 'Не удалось получить базовый каталог репозитория:%1%2'");
...
&НаКлиенте
Процедура ДобавитьГиперссылкиВСтрокуТаблицы(СтрокаТаблицы)
	
	// Здесь ваша логика
	//Если СтрНачинаетсяС(СтрокаТаблицы.Комментарий, "#") Тогда
	//	Элементы.КоммитыКомментарий.ГиперссылкаЯчейки = Истина;
	//КонецЕсли;
...
Процедура ЗаполнитьТаблицуКоммитов() Экспорт
	...
	ЗаполнитьБазовыйКаталогРепозитория();
	...
	Если Результат.КодВозврата = 0 Тогда
		...
		Для НомерСтроки = 1 По СтрокВЛоге Цикл
			...
			Иначе
				...
				ДобавитьГиперссылкиВСтрокуТаблицы(СтрокаТаблицы);
			КонецЕсли;
...
&НаКлиенте
Функция ПолучитьСвойствоКонфигурации(Конфигурация, Путь)
	
	Свойство = Конфигурация.Получить(Путь);
...
&НаКлиенте
Функция ПолучитьИнформациюОКонфигурации(Путь)
	
	Конфигурация = Новый Структура("ЭтоРасширение, Имя, Синоним, Версия", Ложь);
	
	ИмяФайла = СтрЗаменить(Путь, "/", РазделительПути);
	ИмяФайла = БазовыйКаталогРепозитория + РазделительПути + ИмяФайла;
	...
	Если Файл.Существует() Тогда
		...
		Попытка
			...
			Данные = ФабрикаXDTO.ПрочитатьXML(Чтение).Получить("Configuration/Properties");
			Чтение.Закрыть();
			Конфигурация.Имя = ПолучитьСвойствоКонфигурации(Данные, "Name");
			Конфигурация.Синоним = ПолучитьСвойствоКонфигурации(Данные, "Synonym/item/content");
			Конфигурация.Синоним = ?(ЗначениеЗаполнено(Конфигурация.Синоним), Конфигурация.Синоним, Конфигурация.Имя);
			Конфигурация.Версия = ПолучитьСвойствоКонфигурации(Данные, "Version");
			Конфигурация.ЭтоРасширение = Данные.Свойства().Получить("ConfigurationExtensionPurpose") <> Неопределено;
		Исключение
			ТекстОшибки = НСтр("ru = 'Не удалось прочитать информацию о конфигурации из файла: '");
			ТекстОшибки = ТекстОшибки + ИмяФайла;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		КонецПопытки;
...
&НаКлиенте
Функция ОпределитьКаталогКонфигурацииПоВыводуGit(ТекстВывода)
	...
	Для НомерСтроки = 1 По СтрЧислоСтрок(ТекстВывода) Цикл
		...
		Путь = СтрПолучитьСтроку(ТекстВывода, НомерСтроки);
		
		Если СтрНайти(Путь, "Configuration.xml") = 0 Тогда
			Продолжить;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(КаталогКонфигурации) Тогда
			КаталогКонфигурации = КаталогКонфигурации + Символы.ПС;
		КонецЕсли;
		
		ЧастиПути = СтрРазделить(Путь, "/");
		ЧастиПути.Удалить(ЧастиПути.Количество() - 1);
		Каталог = СтрСоединить(ЧастиПути, "/") + "/";
		КаталогКонфигурации = КаталогКонфигурации + Каталог;
		
		Конфигурация = ПолучитьИнформациюОКонфигурации(Путь);
		Конфигурации[Каталог] = Конфигурация;
		
	КонецЦикла;
	
	ДанныеКонфигураций = Новый ФиксированноеСоответствие(Конфигурации);
...
Функция ОпределитьКаталогКонфигурации(ХешКоммита)
	...
	Если Результат.КодВозврата = 0 И ЗначениеЗаполнено(Результат.ТекстВывода) Тогда
		КаталогКонфигурации = ОпределитьКаталогКонфигурацииПоВыводуGit(Результат.ТекстВывода);
	КонецЕсли;
...
&НаКлиенте
Функция ПолучитьВерсиюХранилищаИзФайла(ХешКоммита)
	
	ВерсияХранилища = 0;
	Файлы = ПолучитьИзмененныеФайлыПоКоммиту(ХешКоммита);
	
	Для Каждого Файл Из Файлы Цикл
		
		Если Не СтрЗаканчиваетсяНа(Файл, "VERSION") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьВерсиюХранилища(СтрокаТаблицы)
	...
	Если Объект.ОпределятьВерсиюХранилища Тогда
		ВерсияХранилища = ПолучитьВерсиюХранилищаИзФайла(ХешКоммита);
	КонецЕсли;
...
&НаКлиенте
Функция ПолучитьБазовуюИнформациюОФайле(Файл)
	
	Если Не ЗначениеЗаполнено(ТекущийКаталогКонфигурации) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ИнформациюОФайле = Неопределено;
	НомерСтроки = 1;
	ВсегоСтрок = СтрЧислоСтрок(ТекущийКаталогКонфигурации);
	
	Пока НомерСтроки <= ВсегоСтрок И ИнформациюОФайле = Неопределено Цикл
		Каталог = СтрПолучитьСтроку(ТекущийКаталогКонфигурации, НомерСтроки);
		Если СтрНачинаетсяС(Файл, Каталог) Тогда
			Путь = СтрЗаменить(Файл, Каталог, "");
			ИнформациюОФайле = Новый Структура();
			ИнформациюОФайле.Вставить("КаталогКонфигурации", Каталог);
			ИнформациюОФайле.Вставить("Конфигурация", ДанныеКонфигураций.Получить(Каталог));
			ИнформациюОФайле.Вставить("ПолныйПуть", СтрЗаменить(Файл, Каталог, ""));
			ИнформациюОФайле.Вставить("Путь", "");
		КонецЕсли;
		НомерСтроки = НомерСтроки + 1;
	КонецЦикла;
	
	Возврат ИнформациюОФайле;
	
КонецФункции

&НаКлиенте
Функция ПолучитьПредставлениеОбъектаМетаданных(ИнформацияОФайле)
	
	Представление = СтрШаблон("%1.%2", ИнформацияОФайле.ИмяОбъекта, ИнформацияОФайле.ЧастиПути[1]);
	
	Если 1 < ДанныеКонфигураций.Количество() Тогда
		
		Если ИнформацияОФайле.Конфигурация <> Неопределено И ИнформацияОФайле.Конфигурация.ЭтоРасширение Тогда
			Представление = СтрШаблон("%1 (расш. ""%2"")", Представление, ИнформацияОФайле.Конфигурация.Синоним);
		КонецЕсли;
...
&НаКлиенте
Функция ПолучитьИнформациюОФайле(Файл)
	
	ИнформацияОФайле = ПолучитьБазовуюИнформациюОФайле(Файл);
	
	Если ИнформацияОФайле = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Путь = Лев(ИнформацияОФайле.ПолныйПуть, СтрДлина(ИнформацияОФайле.ПолныйПуть) - 4);
	...
	Если 0 < ЧастиБезИмени.Количество() Тогда
		...
		ИнформацияОФайле.Путь = СтрШаблон("%1.%2.%3", ЧастиПути[0], ЧастиПути[1], ПутьБезИмени);
	Иначе
		ИнформацияОФайле.Путь = СтрЗаменить(Путь, "/", ".");
	КонецЕсли;
	
	ИнформацияОФайле.Вставить("ЧастиПути", СтрРазделить(ИнформацияОФайле.Путь, "."));
	
	ВидОбъекта = ИнформацияОФайле.ЧастиПути[0];
	ИмяОбъекта = ОпределитьИмяОбъектаМетаданных(ВидОбъекта);
	
	ИнформацияОФайле.Вставить("ВидОбъекта", ВидОбъекта);
	ИнформацияОФайле.Вставить("ИмяДляСортировки", ИмяОбъекта);  
	
	Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
		ИнформацияОФайле.Вставить("ИмяОбъекта", Сред(ИмяОбъекта, 4));
	Иначе
		ИнформацияОФайле.Вставить("ИмяОбъекта", "");
	КонецЕсли;
	
	ПредставлениеОбъекта = ПолучитьПредставлениеОбъектаМетаданных(ИнформацияОФайле);
	ИнформацияОФайле.Вставить("ПредставлениеОбъекта", ПредставлениеОбъекта);
...
Функция ПолучитьОписаниеОбъектаМетаданныхПоИмениФайл(Файл, КоммитыДляСравнения)
	...
	ИнформацияОФайле = ПолучитьИнформациюОФайле(Файл);
	
	Если ИнформацияОФайле = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ИнформацияОФайле.ИмяОбъекта) Тогда
	...
	Если ЗначениеЗаполнено(ИнформацияОФайле.ИмяОбъекта) Тогда
		
		ИзмененияСвойств = ПолучитьИзмененияСвойствОбъекта(Файл, КоммитыДляСравнения);
		Картинка = ПолучитьКартинкуОбъектаМетаданных(ИнформацияОФайле.ИмяОбъекта);
		
		Описание = Новый Структура();
		Описание.Вставить("ИмяОбъекта", ИнформацияОФайле.ПредставлениеОбъекта);
		Описание.Вставить("ИмяДляСортировки", ИнформацияОФайле.ИмяДляСортировки);
		Описание.Вставить("Файл", Файл);
		Описание.Вставить("Путь", ИнформацияОФайле.Путь);
		...
		Для Индекс = 2 По ИнформацияОФайле.ЧастиПути.Количество() - 1 Цикл
			
			Часть = ИнформацияОФайле.ЧастиПути[Индекс];
